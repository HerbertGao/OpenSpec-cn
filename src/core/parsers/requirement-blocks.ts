...
// REGEX captures the name; PREFIX is the header boundary used by the seek loops.
...
function parseRequirementBlocksFromSection(
  ...
  while (i < lines.length) {
    ...
    // A line the PREFIX matched but the REGEX did not is a nameless header
    // (e.g. "### Requirement:"); record it as a skipped header so the INFO note
    // still fires under bilingual parsing.
    if (!m) { recordIfSkippedHeader(i); i++; continue; }
